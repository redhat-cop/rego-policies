...
@test "policy/ocp/bestpractices/common-k8s-labels-notset" {
  ...
  cmd="opa eval --input ${tmp}/list.yml --data policy/lib --data ${policy_dir}/src.rego --profile --format pretty ${policy_package}"
  run ${cmd}
...
@test "policy/podman/history/contains-layer" {
  ...
  cmd="opa eval --input ${tmp}/jenkins-python-mising.json --data policy/lib --data policy/podman/data_parameters.rego --data ${policy_dir}/src.rego --profile --format pretty ${policy_package}"
  run ${cmd}
...
@test "policy/podman/images/image-size-not-greater-than" {
  ...
  cmd="opa eval --input ${tmp}/jenkins-base.json --data policy/lib --data policy/podman/data_parameters.rego --data ${policy_dir}/src.rego --profile --format pretty ${policy_package}"
  run ${cmd}
